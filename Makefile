...
TESTL  =  /home/jose/anaconda2/pkgs/graphviz-2.38.0-2/lib
...
$(TARGET_LIB) : $(OBJECTS_LIB)
	mkdir -p lib
	$(CC) $(CFLAGS) -L$(LIBDIR) -L$(TESTL) -Wl,-rpath $(LIBDIR) -o $@ $(OBJECTS_LIB) $(LDFLAGS) $(LDLIBS)

.PHONY: test
test : $(TARGET_TEST)
tests/%.out: tests/%.c
	$(CC) $(CFLAGS) -L$(LIBDIR) -L$(TESTL) -Wl,-rpath $(LIBDIR) -o $@ $< -lpfnet $(LDLIBS)
