...
r_test=load_data['wind']*total_load*(penetration/100)/num_r
if int(sys.argv[4])==6:
...
def eval_on_off(on_off_value,r,j,prob,Constraints,duals=False,solver=ECOS):
    ...
    if solver==GUROBI:
        #try:    
        Q=prob.solve(solver=GUROBI,BarQCPConvTol=1e-5,verbose=True,Method=4)
        #except:
        #    print "Gurobi Failed"
        #    Q=prob.solve(solver=ECOS,reltol=1e-8,max_iters=1000,warm_start=True)
    else:
        try:
            Q=prob.solve(solver=ECOS,reltol=1e-8,max_iters=1000,verbose=False)#warm_start=True
        except:
            print "Failed ECOS"
            Q=np.nan
...
if not CONT:
    Objective_approx=0
    Constraints_approx=[]
    Objective_approx, Constraints_approx=second_stage(on_off_value=on_off_param,PWL=False) #quad 
    prob_approx=[]
...
if benders:
    ...
    for k in range(sim_step+1):
        ...
        pool=Pool(24,initargs=(on_off_value,))
...
delta_uk0=[]
k0=50
n_par=int(str(sys.argv[6]))
sim_step=sim_step*n_par
iter_time_total=[]
if not benders:
    iter_time=time.time()
    if CONT:
    ...
    for k in range (sim_step/n_par):
        ...
        if not CONT:
            print "solving MILP",
            F_eval=prob_det.solve(solver=GUROBI,MIPGap=1e-3,warm_start=True,verbose=False)#GUROBI,MIPGap=1e-1,warm_start=True
        else:
        ...
        print "Solving QP"
        pool=Pool(5,initargs=(on_off_value,))
        res = list(zip(*pool.map(lambda i: eval_on_off(on_off_value=on_off_value,
                                                       r=scenarios_bus[:,:,n_par*m:n_par*(m+1)], # 0:sim_step
                                                       j=i,prob=prob,Constraints=Constraints,duals=True,solver=ECOS),
                                                       range(0,n_par),chunksize=1)))
        pool.close()
        pool.join()
        Q_value_stoch=list(res[0])
        M1_stoch=list(res[1])
        ...
        elif not CONT and  ((not np.all(on_off_array_cont[k]==on_off_array_cont[k-1]))  or k==0):#(k%5==0 or k==(sim_step/n_par)-1):
            on_off_array.append(on_off_value)
            time_eval.append(time.time()-st)
            st=time.time()
            k_save.append(k)
...
if n_par==1:
    ...
    series='parallel'
save_path="pickle/pegase/uptime/Stoch_"+name+"_pegase_"+str(sim_step)+"iter_3pwl_"+"relaxation_"+str(CONT)+ "_cluster_warm_start_1_alpha"+str(k0)+"norm_u_"+str(u_bound)+"_gamma_"+str(sys.argv[2])+"_evalgamma_"+str(sys.argv[3])+"_day_"+str(sys.argv[4])+"_mult_"+str(sys.argv[5])+series+"_quad_chunk"+".p"
print save_path
