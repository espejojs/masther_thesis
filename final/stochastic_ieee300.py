...
for i in range(len(b_list)):#reversed(range(0,len(gen_bounds)-1)): 
    for gen in net.generators:
        if gen.index in a_list[b_list[i]]:#gen.P_max >= gen_bounds[i+1] and gen.P_max < gen_bounds[i]:
...
if not benders:
    ...
    for k in range (sim_step/n_par):
        ...
        elif not CONT and ((not np.all(on_off_array_cont[k]==on_off_array_cont[k-1]))  or k==0):
            on_off_array.append(on_off_value)
            time_eval.append(time.time()-st)
            st=time.time()
            k_save.append(k)
...
if benders:
    ...
    name="hybrid"
save_path="pickle/ieee300/uptime/Stoch_"+name+"_ieee300_"+str(sim_step)+"iter_"+str(pwl_points)+"pwl_"+"relaxation_"+str(CONT)+ "_cluster_warm_start_1_alpha"+str(k0)+"norm_u_"+str(u_bound)+"_gamma_"+str(sys.argv[2])+"_evalgamma_"+str(sys.argv[3])+"_day_"+str(sys.argv[4])+"_mult_"+str(sys.argv[5])+"_parallel_newmix_quad_bugfix"+".p"
print save_path
